...
function ocpng.loadPNG(fname)
    ...
    while true do
        ...
        if ctyp == "IHDR" then
            ...
            if png_inter ~= 0 then
                error("we don't support interlacing (yet)")
            end
        ...
        elseif ctyp == "PLTE" then
            ...
            local i
            for i = 0, #cdat // 3 - 1 do
                PALETTE[i + 1] = (0 + (cdat:byte(3 * i + 1, 3 * i + 1) << 16) + (cdat:byte(3 * i + 2, 3 * i + 2) << 8) +
        ...
        elseif ctyp == "tRNS" then
            ...
            elseif png_cm == 2 and png_bpc == 8 then
                -- rgb + transparency
                for i = 0, #cdat // 3 - 1 do
                    png_trns[(cdat:byte(3 * i + 1, 3 * i + 1) << 16) + (cdat:byte(3 * i + 2, 3 * i + 2) << 8) +
    ...
    for y = 0, png_h - 1 do
        ...
        elseif ftyp == 1 then
            ...
            for x = 2 + png_fstride, #line do
                unpacked = unpacked .. string.char(0xFF & (line:byte(x) + unpacked:byte(#unpacked - png_fstride + 1)))
            end

        elseif ftyp == 2 then
            -- dy
            for x = 2, #line do
                unpacked = unpacked .. string.char(0xFF & (line:byte(x) + unpacked:byte(#unpacked - png_bwidth + 1)))
            end
        ...
        elseif ftyp == 3 then
            ...
            for x = 2, 2 + png_fstride - 1 do
                unpacked = unpacked ..
                               string.char(0xFF & (line:byte(x) + (unpacked:byte(#unpacked - png_bwidth + 1) >> 1)))
            end
            for x = 2 + png_fstride, #line do
                unpacked = unpacked .. string.char(0xFF &
                                                       (line:byte(x) +
                                                           ((unpacked:byte(#unpacked - png_bwidth + 1) +
                                                               unpacked:byte(#unpacked - png_fstride + 1)) >> 1)))
            end
        ...
        elseif ftyp == 4 then
            ...
            for x = 2, 2 + png_fstride - 1 do
                unpacked = unpacked ..
                               string.char(
                        0xFF & (line:byte(x) + paeth(0, unpacked:byte(#unpacked - png_bwidth + 1), 0)))
            end
            for x = 2 + png_fstride, #line do
                unpacked = unpacked .. string.char(0xFF &
                                                       (line:byte(x) +
                                                           paeth(unpacked:byte(#unpacked - png_fstride + 1),
                            unpacked:byte(#unpacked - png_bwidth + 1), unpacked:byte(
                                #unpacked - png_bwidth + 1 - png_fstride))))
            end
    ...
    return {
        ...
        get = getter,
        getAlpha = getalpha,
        isPaletted = function(self)
            return self.png_cm == 3
        end,
